...
"""
function spread(pg::PowerGrid, rpg::ODEFunction, indicies, op::State, ur_idx, ui_idx, nodes, dist_args, tau_max::Float64, throws::Int64)
    ...
    for n in 1:length(pg.nodes)
            ...
            # getting the dispalcement after perturbing "node"
            Δx = z_new .- op.vec
...
function spread_parallel(pg::PowerGrid, op, dist_args, tau_max::Float64, sample_size::Int64)
    ...
    result = pmap(sample -> spread(pg, rpg, indicies, op, ur_idx, ui_idx, nodes, dist_args, tau_max, sample), 1:sample_size)
    Δx_abs = mean(result)
    ...
    for n in 1:length(pg.nodes)
        ...
        if typeof(pg.nodes[nodes[n]]) == SlackAlgebraic
            ...
            A_x[n] = sum(Δx_abs[n, :])
            # getting the shotest path legnth to "node" for all verticies in the network
...
function preliminary_spread(pg::PowerGrid)
    ...
    indicies = map(n -> pd_node_idx(pg, rpg, n, "Voltage"), 1:length(pg.nodes))

    # handeling the type of the pg
    ...
    return rpg, ur_idx, ui_idx, nodes, indicies
end